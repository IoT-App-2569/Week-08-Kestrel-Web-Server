...
public class DualSerialBridgeWorker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                // เปลี่ยน COM1 เป็น COM3 ในการ Fallback ถ้าจำเป็น แต่ในโค้ดนี้ตั้งให้ข้าม COM1
                ...
                try
                {
                    ...
                    while (!stoppingToken.IsCancellationRequested && serial.IsOpen)
                    {
                        if (serial.BytesToRead > 0)
                        {
                            ...
                            // *** แก้ปัญหาหน่วง: ถ้าคิวข้อมูลยาวเกินไป ให้เคลียร์ทิ้งเพื่อเอาค่าล่าสุดเสมอ ***
                            if (serial.BytesToRead > 1024) 
                            {
                                serial.DiscardInBuffer();
                            }
