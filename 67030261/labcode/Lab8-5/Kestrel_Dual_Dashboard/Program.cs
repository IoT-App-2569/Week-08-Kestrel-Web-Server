    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            if (availablePorts.Length > 0)
            {
                ...
                // หมายเหตุ: บน macOS ให้ใช้พอร์ตแบบ /dev/cu.* (callout) แทน /dev/tty.*
                // เพราะ /dev/tty.* จะรอสัญญาณ Carrier Detect (DCD) ก่อนเปิดได้ ทำให้ SerialPort.Open() ล้มเหลว
                string? targetPort = "/dev/tty.usbserial-0001"; // <-- ใส่พอร์ตที่ต้องการ หรือตั้งเป็น null เพื่อให้ออโต้
            ...
            for (int i = 0; i < 20 && !stoppingToken.IsCancellationRequested; i++)
            {
                ...
                int sA = (int)((Math.Sin(t * 0.8) + 1.0) / 2.0 * 4095);
                int sB = (int)((Math.Cos(t * 1.4) + 1.0) / 2.0 * 4095);
