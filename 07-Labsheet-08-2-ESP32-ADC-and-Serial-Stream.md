...
1. ใน ESP-IDF v6.x ฟังก์ชัน `adc_oneshot_read()` คืนค่าผลลัพธ์เป็นตัวเลขแบบใด และมีช่วงค่าต่ำสุด-สูงสุดเท่าใดเมื่อตั้งค่าเป็น `ADC_BITWIDTH_12`
   - **คำตอบ:** คืนค่าเป็น ตัวเลขจำนวนเต็ม (Integer) แบบค่าดิบ (Raw Value) ช่วงค่าคือ 0 ถึง 4095 (ความละเอียด 12-bit / 4,096 ระดับ)
1. ทำไมเราจึงต้องกดปุ่ม `Ctrl + ]` เพื่อปิด **ESP-IDF Monitor** ก่อนที่เราจะรันโปรแกรมฝั่ง C# Kestrel ในใบงานถัดไป
   *(คำใบ้: หากโปรเซส idf.py monitor ยังเปิดค้างอยู่ จะเกิดอะไรขึ้นกับพอร์ต COM)*
   - **คำตอบ:** (การกด Ctrl + ]) เพื่อสั่งยกเลิกการครอบครองพอร์ตสื่อสารอนุกรม (COM Port) เนื่องจากพอร์ตสื่อสารอนุญาตให้โปรเซสเข้าถึงได้ทีละ 1 โปรเซสเท่านั้น (Exclusive Access) หากไม่ปิด จะทำให้โปรแกรม C# Kestrel เชื่อมต่อพอร์ต COM ในแลปถัดไปไม่ได้
