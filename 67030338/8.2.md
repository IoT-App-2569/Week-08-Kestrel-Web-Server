...
1. ใน ESP-IDF v6.x ฟังก์ชัน `adc_oneshot_read()` คืนค่าผลลัพธ์เป็นตัวเลขแบบใด และมีช่วงค่าต่ำสุด-สูงสุดเท่าใดเมื่อตั้งค่าเป็น `ADC_BITWIDTH_12`
   - **คำตอบ:** คืนค่าเป็นตัวเลขจำนวนเต็ม (Integer / `int`) โดยมีช่วงค่าต่ำสุด–สูงสุดตั้งแต่อย่างน้อย **0 ถึง 4095** (รวม $2^{12} = 4,096$ ระดับ)
1. ทำไมเราจึงต้องกดปุ่ม `Ctrl + ]` เพื่อปิด **ESP-IDF Monitor** ก่อนที่เราจะรันโปรแกรมฝั่ง C# Kestrel ในใบงานถัดไป
   *(คำใบ้: หากโปรเซส idf.py monitor ยังเปิดค้างอยู่ จะเกิดอะไรขึ้นกับพอร์ต COM)*
   - **คำตอบ:** เพราะพอร์ตสื่อสารอนุกรม (COM Port) สามารถถูกใช้งานโดยโปรเซสได้ทีละ 1 โปรเซสเท่านั้น (Exclusive Lock) หากเปิด `idf.py monitor` ค้างไว้ จะทำให้โปรแกรมฝั่ง C# ไม่สามารถเปิดเข้าใช้งานพอร์ต COM เดียวกันได้ และจะเกิดข้อผิดพลาด Access Denied / Port in Use
...
<img width="1030" height="685" alt="Screenshot 2026-09-10 210000" src="https://github.com/user-attachments/assets/0b8caa4a-9f92-4803-8880-202da3dec4da" />
<img width="867" height="607" alt="Screenshot 2026-09-10 210057" src="https://github.com/user-attachments/assets/f3646845-0d65-4f78-a038-c677e7348e37" />
