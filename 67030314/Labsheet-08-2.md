...
1. ใน ESP-IDF v6.x ฟังก์ชัน `adc_oneshot_read()` คืนค่าผลลัพธ์เป็นตัวเลขแบบใด และมีช่วงค่าต่ำสุด-สูงสุดเท่าใดเมื่อตั้งค่าเป็น `ADC_BITWIDTH_12`
   - **คำตอบ:** คืนค่าผลลัพธ์เป็นตัวเลขจำนวนเต็ม แบบความละเอียดดิบ มีช่วงค่าต่ำสุดคือ 0 และสูงสุดคือ 4095 เมื่อตั้งค่าเป็น ADC_BITWIDTH_12
2. ทำไมเราจึงต้องกดปุ่ม `Ctrl + ]` เพื่อปิด **ESP-IDF Monitor** ก่อนที่เราจะรันโปรแกรมฝั่ง C# Kestrel ในใบงานถัดไป
   *(คำใบ้: หากโปรเซส idf.py monitor ยังเปิดค้างอยู่ จะเกิดอะไรขึ้นกับพอร์ต COM)*
   - **คำตอบ:** หากเปิด idf.py monitor ค้างไว้ โปรเซสจะทำการล็อกพอร์ต COM ไว้เพียงผู้เดียว ทำให้โปรแกรมฝั่ง C# Kestrel ไม่สามารถเข้าถึงหรือเปิดเชื่อมต่อพอร์ต Serial นั้นได้ และจะเกิดข้อผิดพลาด Access Denied / Serial Port In Use ในทันที
...
![[Pasted image 20260907100736.png]]
![[Pasted image 20260907102539.png]]